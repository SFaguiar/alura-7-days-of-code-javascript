...
alert(`Front-end: 'a programming area dedicated to creating the visual and interactive part of a website, application or software.
It is what the user sees and uses when accessing a digital platform' [Kenzie 2023].`)
alert(`Back-end: 'Backend developers mainly focus on how the website works.
They write code focused on the functionality and logic that makes the web application they work on work, and the technology they use is never seen by the users accessing the web application' [Harve].`)

while (true) {
    let pill = prompt(`So, you are going to choose the back-pill or the front-pill? Type 1 for back-end and 2 for front-end.`)
    if (pill == 1) {
...
if (choice == 1) {
    ...
    all_courses = chosen_courses.concat(remaining_courses)
} else if (choice == 2) {
...
alert(`You've learned ${chosen_courses.length} so far, which are ${chosen_courses}.`)
...
while (choice === "ok"){
    ...
    chosen_courses.push(newTechnology)
